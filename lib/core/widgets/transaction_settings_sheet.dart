...
class TransactionSettingsSheet extends StatefulWidget {
  ...
  @override
  State<TransactionSettingsSheet> createState() =>
      _TransactionSettingsSheetState();
}
...
class _TransactionSettingsSheetState extends State<TransactionSettingsSheet> {
  ...
  void initState() {
    ...
    _evidenceFuture = context.read<VaultService>().getLinkedDocuments(
  ...
  Future<void> _fetchMatches() async {
    ...
    try {
      ...
      final url =
          Uri.parse(
            '${config.backendUrl}/api/v1/mobile/ingestion/matches',
          ).replace(
            queryParameters: {
      ...
      try {
        ...
        uploadResult.fold(
          ...
          (_) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Evidence uploaded and linked!'),
    ...
    try {
      ...
      final response = await http.put(
        ...
        body: jsonEncode({
          ...
          if (_isTransfer && _targetAccountId != null)
            'to_account_id': _targetAccountId,
          if (_isTransfer && _linkedTransactionId != null)
            'linked_transaction_id': _linkedTransactionId,
        }),
      );
                  ...
                  if (_isLoadingMatches)
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          'SUGGESTED MATCHES (${_matches.length})',
