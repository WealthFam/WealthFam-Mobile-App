...
class InvestmentGoalDetailsScreen extends StatefulWidget {
  ...
  State<InvestmentGoalDetailsScreen> createState() =>
      _InvestmentGoalDetailsScreenState();
}
...
class _InvestmentGoalDetailsScreenState
  ...
  Future<void> refreshData() async {
    ...
    try {
      final response = await http.get(
        Uri.parse(
          '${config.backendUrl}/api/v1/mobile/investment-goals/${_goal['id']}',
        ),
  ...
  Widget build(BuildContext context) {
    ...
    final target =
        double.tryParse(_goal['target_amount']?.toString() ?? '0') ?? 0.0;
    final current =
        double.tryParse(_goal['current_amount']?.toString() ?? '0') ?? 0.0;
    final progress = target > 0 ? (current / target).clamp(0.0, 1.0) : 0.0;
    final remaining =
        double.tryParse(_goal['remaining_amount']?.toString() ?? '0') ?? 0.0;
        ...
        children: [
          ...
          Row(
            children: [
              _buildMetric(
                'Current',
                '$currency${(current / maskingFactor).toStringAsFixed(0)}',
              ),
              const Spacer(),
              _buildMetric(
                'Target',
                '$currency${(target / maskingFactor).toStringAsFixed(0)}',
              ),
              const Spacer(),
              _buildMetric(
                'Remaining',
                '$currency${(remaining / maskingFactor).toStringAsFixed(0)}',
              ),
            ],
          ),
        ],
  ...
  Widget _buildAssetTile(dynamic a, String currency) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          a['type']?.toString().replaceAll('_', ' ') ?? 'Manual',
          style: const TextStyle(fontSize: 12),
        ),
        trailing: Text(
          '$currency${(val / maskingFactor).toStringAsFixed(0)}',
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              final success = await context.read<GoalsService>().deleteGoal(
                _goal['id'].toString(),
              );
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              final success = await context.read<GoalsService>().updateGoal(
                _goal['id'].toString(),
                {
          ...
          TextButton(
            onPressed: () async {
              ...
              final response = await http.post(
                Uri.parse(
                  '${config.backendUrl}/api/v1/mobile/investment-goals/unlink',
                ),
                headers: {
      ...
      builder: (context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: available.isEmpty
              ? const Text(
                  'All your funds are already linked or no funds found.',
                )
              : ListView.builder(
                  shrinkWrap: true,
                    ...
                    return ListTile(
                      ...
                      onTap: () async {
                        final linkRes = await http.post(
                          Uri.parse(
                            '${config.backendUrl}/api/v1/mobile/investment-goals/${_goal['id']}/link',
                          ),
                          headers: {
                            'Authorization': 'Bearer ${auth.accessToken}',
                            'Content-Type': 'application/json',
